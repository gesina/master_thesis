...
\begin{Cor}\label{thomclassadditivity} % TODO: Lemma instead? Join with above Cor?
  The Thom class construction is additive, \idest
  for vector bundles $\xi\colon E\to B$, $\eta\colon E'\to B$
  of rank $i$ respectively $j$ over a paracompact space $X$, and Thom
  classes
  $\u(\xi)\in H^i(\E\xi, \E\xi\setminus\zerosec{\E\xi}; R)$,
  $\u(\eta)\in H^i(\E\eta, \E\eta\setminus\zerosec{\E\eta}; R)$
  the class
  \begin{gather*}
    \u(\xi)\times\u(\eta)=\pb \pi_\xi\u(\xi) \cup \pb \pi_\eta\u(\eta)
    \in H^{i+j}(\E{(\xi\oplus\eta)},
    \E{(\xi\oplus\eta)}\setminus\zerosec{\E{(\xi\oplus\eta)}})
  \end{gather*}
  is a Thom class of $\xi\oplus\eta$.
  For this representation the canonical isomorphism
  $\E{(\xi\oplus\eta)}\cong\E\xi\oplus\E\eta$
  and the corresponding Künneth isomorphism % TODO: Ref
  \begin{align*}
    H^i(\E\xi, \E\xi\setminus\zerosec{\E\xi})
      \otimes
      H^j(\E\eta, \E\eta\setminus\zerosec{\E\eta})
    &\overset{\sim}\longto
      H^{i+j}(\E{(\xi\oplus\eta)},
      \E{(\xi\oplus\eta)}\setminus\zerosec{\E{(\xi\oplus\eta)}})\\
    x\otimes y
    &\longmapsto
  ...
  was used.
  \begin{proof}
    % TODO: proof additivity Thom class
  \end{proof}
\end{Cor}
...
\begin{Thm}
  ...
  \begin{proof}
    ...
    \item[Additivity:] % TODO: additivity alt def SW-classes
      Consicer vector bundles $\xi$, $\eta$ over a paracompact space
      $B$. With $\u(\xi\oplus\eta)=\u(\xi)\cup\u(\eta)$ get
      \begin{align*} % TODO: CHECK
        \thomiso(\w_i(\xi\oplus\eta))
        &= \Sq i(\u(\xi\oplus\eta)) \\
        &\overset{\mathllap{\text{\autoref{thomclassadditivity}}}}=
