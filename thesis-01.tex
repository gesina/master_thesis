...
\begin{Not}
  ...
  $\Sq{i_1}\cdot\Sq{i_2}\dotsm\Sq{i_r}$ will have the short form
  $\Sq{(i_1,i_2,\dotsc,i_r)}$,
  and will be evaluated on an element $x$ of a cohomology ring as
  $\Sq{i_1}\circ\dotsb\circ\Sq{i_r}(x)$ respecting the
  properties from \autoref{def:sq}.
  Furthermore, for a sequence $I=(i_1,\dotsc,i_r)$,
  respectively $\Sq I$, denote by
  \begin{description}
  \item[$\l(I)\coloneqq r$] the \emph{length} of $I$,
  \item[$\d(I)\coloneqq \sum_{j=0}^{r} i_j$] the \emph{degree} of $I$,
    and by
  \item[$\e(I)\coloneqq 2i_1-\d(I)=\sum_{j=1}^{r-1}(i_j-2i_{j+1})$]
...
\begin{Rem}
  ...
  \end{itemize}
\end{Rem}
\begin{Cor}\label{cor:thomclassmultiplicative} % TODO: Lemma instead? Join with above Cor?
...
\begin{Thm}[Thom isomorphism]
  ...
  \begin{proof} % TODO: Ref/proof naturality Thom iso
...
\begin{Thm}\label{thm:altdefswclasses}
  ...
  $\ws{i}=\pb\incl\w{i}{\gamma_n}\in \H^i(\BO)$. % TODO: ref
  \begin{proof}
...
\begin{Thm}\label{thm:altdefwuclasses}
  ...
  \begin{proof}
    ...
    \item[By {\cite[Chap.~3.3.2, p.~241]{hatcher}}
      (compare \eqref{eq:capprod2}):] % TODO: join eq:capprod2 equations
