...
fullfilling the conjecture whose cobordism classes form generating set
of the cobordism ring.
As the cobordism ring has the form of a polynomial algebra
$\Zmod2[\sigma_i|i\neq 2^r-1]$, a set of elements
$([\G i]|i\neq2^r-1)$ will already be a generating set if all of the
$\G i$ are indecomposable.
Thus, when candidates for the generating set are constructed in
\autoref{sec:proofbrown}, it remains to check (see
Theorem~\autoref{thm:brownproof}):
\begin{enumerate}
\item that they are indecomposable (This is the harder part and
  requires the preliminary work in
  \autoref{sec:indecomposabilitycriterion}), and
\item that they fullfil the immersion conjecture.
\end{enumerate}
...
Write the equivalence, respectively cobordism, class of a manifold $M$
by $[M]$.
...
\begin{Thm}[Thom]\label{thm:cobordismringstructure} % TODO: cobordism ring thm more elaborate?
...
and can be proven using the results from the preceeding subsections.
...
% TODO: Recall needed results from cobordism ring
\begin{proof}[proof of \autoref{thm:indecomposabilitycriterion}]
  ...
  \item\label{item:manifoldbasisrepr}
    ...
    \end{gather*}
    of the classes $[V_\Part]$ for non-dyadic partitions of $n$.
    Now the Stiefel-Whitney classes and numbers are additive with
    respect to disjoint sum as well as determined by the cobordism
    class according to Theorem~\autoref{thm:cobordantiffswnumscoincide}.
  ...
  \item\label{item:generatorscobordimsring}
    ...
    manifold $V_{(k)}\in\c_k$ for $k\neq 2^m-1$ are indecomposable,
    hence algebraically independent, generators of the cobordism
    ring.
  ...
  \item By \ref{item:generatorscobordimsring}, the bordism
    ...
    basis elements $[V_\Part]$ contains the unique $k$-dimensional
    indecomposable basis element $[V_{(k)}]$ as a summand,
...
\begin{Lem}
  ...
  \begin{proof}
    This is a direct consequence of Lucas' Theorem % TODO: ref
    which states
    \begin{gather*}
...
\begin{Thm}\label{thm:brownproof}
  ...
  \item\label{item:indecomposabilityproperty} The cobordism classes of the $\G m$, $m\neq 2^s-1$, are
...
\begin{proof}[proof of
  ...
  By definition, all $\G m$ are or arise as iterated twisted products from
  some $\Twistedprod{2^i}{\pt}$. Or more precisely, for a binary
  representation
  \begin{align*}
    m &= 2^{i_1}+\dotsb + 2^{i_l} \\
...
\begin{proof}[proof of
  ...
  \begin{center}
    \begin{tikzcd}[row sep=small, column sep=large]
      ...
      \ar[d, equals]
      \ar[r, hookrightarrow, "1\times\iota\times\iota"]
      &
