...
\optcite[Theorem~1]{milgram}\optcite{sanderson}.
...
solved for homology classes in degree of codimension one, \idest:
\begin{Lem}
...
\begin{proof}[proof of
  ...
  As $\T{\G m}\oplus\N{\emb}=\pb\emb\T{K^{n+1}}$ there is the relation
  ...
  \begin{Lem}
    ...
    \begin{proof}
      Skipped.
      \TODO{Ref. for proof (without spectra)?}
    \end{proof}
  \end{Lem}
  By the above Lemma
  \begin{align*}
  ...
  $\fundcl{K^{m+1}}$ is the Poincaré isomorphism, it suffices for the
  proof to show that $y^{m+1}$ is non-zero. But by the relations of the
  $x_r$ combinatorics give that the only non-zero product
...
dimension was chosen correctly. So this is the less problematic issue.
For the immersion property we will again utilise nice immersion
properties of projective spaces, more precisely the following one for
the total space of line bundles over such:
\begin{Lem}[{\cite[Theorem~4.1, p.~418]{mahowald}}]
  \label{lem:immersionuniversalbdl}
  % Wrong attempt \optcite[p.~85]{immersionconj}:
...
\end{Lem}
The trick now will be to find an immersion of each or our candidates
into some $\Twistedprod k (\R^n)$, which is the total space of
$(n\cdot\gamma_k)\oplus\trivbdl^n$. For this, $k$ and $n$ have to be
chosen correctly to not also make the indecomposability criterion
applicable, but also Mahowald's lemma from above.
The following choice will be shown to fulfil this.
\begin{Def}
  Let $m$ be odd and not of the form $2^s-1\in\Nat$ define
  \begin{align*}
    \G 0 &\coloneqq \pt \;, \\ % TODO: to even dimensional gens
    \G m &\coloneqq \Twistedprod{1}{\G {\frac{m-1}{2}}}
      % &\text{
      % where
...
\begin{proof}[proof of
  ...
  Applying Lemma~\ref{lem:immersionuniversalbdl} to $p=2n-1$ and $q=2^{i_1}$
  % TODO: Does not work, q not odd ... !!!
