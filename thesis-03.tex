...
The essential step in relating a twisted product to its lower
dimensional counterpart and the sphere in a useful way is the below
alternative pushout construction.
\begin{Rem}
...
\begin{Rem}
  ...
  %     &\H^l(\Sphere k\times X)
  %     \\
  %     (1\otimes a, s_k\otimes b)
  %     \ar[r, mapsto]
  %     &(1\otimes a, 0)
  %     &(s_k\otimes a, s_k\otimes b)
  %     \ar[r, mapsto]
  %     &(0, s_k\otimes (a+b))\\
  %     &(1\otimes a,s_{k-1}\otimes b)
  %     \ar[r, mapsto]
  %     &(s_k\otimes b, s_k\otimes b)
  %   \end{tikzcd}
  % \end{center}
  % and
  % \begin{center}
...
\begin{Thm}\label{thm:twistedprod:cohomstructure}
  ...
  \begin{align*}
    ...
    &\cong
      ...
      \otimes (N+D)
  \end{align*}
  with the additional relations $c\otimes N=0=s\otimes D$.
  In other words, it is the group
  \begin{align*}
    \H^*(\Twistedprod{k}{X})
    &\cong
      (1\otimes(N+D))
      \oplus \left(\bigoplus_{i=1}^k c^i\otimes D\right)
      \oplus (s\otimes N)
    \\
    &\cong
  ...
  \end{align*}
  with the relation $c\otimes d(a)+c\otimes d(b)=c\otimes d(a+b)$, and
  equipped with a multiplication determined by
  \begin{enumerate}
  \item
    $(1\otimes g) \cdot (1\otimes f)= 1\otimes ab$,
    $(1\otimes g) \cdot (s\otimes f)= s\otimes ab$,
    for $g,f\in N+D$.
  \item
    $(1\otimes d(a))\cdot (c^i\otimes d(b)) = c\otimes d(ab)$, and
    $(1\otimes N) \cdot (c^i\otimes D)\cdot = 0$
    for $a,b\in\H^*(X)$, $1\leq i\leq k$.
  \item 
    $(c\otimes D)\cdot(s_k\otimes N) = 0$.
  \item
    $(c^i\otimes d(1))\cdot(c^j\otimes d(1)) = c^{i+j}\otimes d(1)$
    for $0\leq i+j\leq k$, and $0$ else.
  \item 
    $(s\otimes n)\cdot (s\otimes n')=0$ for any $n,n'\in N$.
  \end{enumerate}
  For readability skip $1\otimes$ and $\otimes d(1)$ in element
  notation wherever it is clear to which part the summand
  belongs.
  Furthermore, $\H^*(\Twistedprod{k}{X})$ fulfils
  \begin{enumerate}[resume]
  \item
    $\pb\pi$ is an isomorphism on
    \begin{align*}
      \pb\pi\colon
    ...
    \begin{align}\label{eq:twistedprodcohom:pi}
      \pb\pi(c)&=0
      &\text{thus}&
      &\pb\pi(c^r\otimes d(a) + 1\otimes n_1 + s\otimes n_2)
      &= 1\otimes n_1 + s_k\otimes n_2
        \;.
    \end{align}
    for any elements $x\in\H^*(X)$, $n_1,n_2\in N$.
  \item
  ...
  \begin{proof}
    ...
    Preservation of injectivity on cohomology is easily explained.
    ...
    written as a sum of $c^i\cdot (1\otimes d(x))$, 
    % TODO: improper reasoning
    one only has to check injectivity of $\pb{\Twistedprod k f}$ on
    the two parts
    $\left( \H^*(\RP k)\otimes 1 \right)$
    and
    $\left(
      (1\otimes D) \oplus (1\otimes N) \oplus (s_k\otimes N)
    \right)$.
