%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Master Thesis in Mathematics
% "Immersions and Stiefel-Whitney classes of Manifolds"
% -- Main File --
%
% Author: Gesina Schwalbe
% Supervisor: Georgios Raptis
% University of Regensburg 2018
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[english]{scrreprt}
\input{thesis-def.tex}

% TITLE
\hypersetup{
  pdfauthor={Gesina Schwalbe},
  pdftitle={Immersions and Stiefel-Whitney Classes of Manifolds}
}
\newfontfamily\fontfamilyfrutiger{Frutiger Next LT W1G} % UR font
\newlength{\urlogowidth}\setlength{\urlogowidth}{11.5cm}
\titlehead{%
  \parbox{2\urlogowidth}{%
    \includegraphics[width=.98\urlogowidth]{UniLogo_Text.jpg}\\%
    \hspace*{0.3333\urlogowidth}%
    {\fontfamilyfrutiger%
      \bfseries\scshape%
      \fontsize{0.041667\urlogowidth}{0.041667\urlogowidth}\selectfont%
      Fakultät für Mathematik}\\[2em]
  }%
}

\subject{Master Thesis}
\title{Immersions and Stiefel-Whitney Classes\\of Manifolds}
\author{Gesina Schwalbe}
\publishers{Supervisor: Dr.~Georgios Raptis}
\newcommand{\handindate}{October~4, 2018}
\date{\handindate}

%\includeonly{thesis-04}
\begin{document}
\pagenumbering{roman}
\maketitle

% Abstract
\include{thesis-abstract}

\tableofcontents
\cleardoubleoddpage
\pagenumbering{arabic}


% Introduction
\include{thesis-00}


% (Re)formulation of the Immersion Conjecture
\include{thesis-01}

% Massey's Theorem
\include{thesis-02}

% Brown's Theorem
\include{thesis-03}

% Outlook
\include{thesis-04}


% Literature
%\printnomenclature
\nocite{*}
\printbibliography


% Erklärung
\include{eigenstaendigkeitserklaerung}

\end{document}

% Local Variables:
% ispell-local-dictionary: "en_US"
% End:
