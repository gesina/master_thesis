...
This chapter is mainly dedicated to show a theorem of Massey that this
obstruction is void in general for $k=n-\alpha(n)$
(\autoref{sec:massey}), and that this is the best possible general
result (\autoref{sec:bestpossibleresult}), \idest for $k<n-\alpha(n)$
there are examples of manifolds not immersing into $\R^{n+k}$.

First start with some preliminary work on Massey's Theorem.
